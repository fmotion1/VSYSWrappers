...
if ($Result -eq 'Yes') {
    ...
    $EmptyDir     = New-TempDirectory
    $EmptyDirName = $EmptyDir.FullName
    $LogDir       = New-TempDirectory
    $LogDirName   = $LogDir.FullName
    $LogFilePath  = [IO.Path]::Combine($LogDirName, "logfile.log")

    robocopy $EmptyDirName $Folder /MIR /R:1 /W:1 /MT:128 /LOG:$LogFilePath

    Remove-Item -LiteralPath $Folder -Force
    Remove-Item -LiteralPath $EmptyDirName -Force
    Remove-Item -LiteralPath $LogDirName -Recurse -Force
