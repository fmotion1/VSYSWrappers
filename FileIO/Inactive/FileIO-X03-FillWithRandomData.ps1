...
@('5','10','20','35','60','120','200','1000','5000','10000','50000','100000') | ForEach-Object {[void] $NumFilesCombo.Items.Add($_)}
...
function GenerateRandomData {
    ...
    if($SelectedExtensions -eq ''){
        Write-Host "Saving random data. File extensions were not passed." -ForegroundColor Cyan
        $saveRandomDataToFilesSplat = @{
            OutputPath = $Path
            FilenameLengthMin = $MinFileLength
            FilenameLengthMax = $MaxFileLength
            FilesizeMin = $SelectedSizesMin
            FilesizeMax = $SelectedSizesMax
            Unit = $SelectedSizesUnit
            NumberOfFiles = $SelectedNumFiles
            RandomFileExtensions = $true
        }

        Save-RandomDataToFiles @saveRandomDataToFilesSplat
    }else{
        Write-Host "Saving random data. File extensions were passed." -ForegroundColor Cyan
        $saveRandomDataToFilesSplat = @{
            OutputPath = $Path
            FilenameLengthMin = $MinFileLength
            FilenameLengthMax = $MaxFileLength
